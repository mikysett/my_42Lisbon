...
/*   Updated: 2021/04/01 15:33:02 by msessa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_size	ft_init_res(t_list **map_params)
{
	...
	return (((t_map_par *)(param_sel->content))->val.res);
}
...
bool	ft_init_game(t_game *game, t_map *map)
{
	...
	res = ft_init_res(map->map_params);
	game->win = mlx_new_window(game->mlx, res.x, res.y, "cub3D");
	ft_mm_init_img(game);
	game->mm_pos = ft_mm_set_pos(res, game->mini_map.size);
