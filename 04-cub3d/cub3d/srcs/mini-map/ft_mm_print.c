...
/*   Updated: 2021/04/22 19:09:39 by msessa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_mini_map *mm, t_img_data *img)
{
	...
	ft_draw_rect(img,
		(t_size){.x = half_size_x - half_p_size,
		.y = half_size_y - half_p_size},
		(t_size){.x = PLAYER_SIZE, .y = PLAYER_SIZE}, CLR_PLAYER);
	dir_point = ft_take_dir_point(player->dir);
	ft_draw_rect(img,
		(t_size){.x = half_size_x + dir_point.x - 1,
		.y = half_size_y - dir_point.y},
